...
std::shared_ptr<Edge> parse_edge(std::string_view edge,
                                 const std::map<std::string_view, std::shared_ptr<Edge>> &edge_cache);
...
void GraphBuilder::relation_collector(const std::vector<std::string_view> &finds) {
    ...
    std::shared_ptr<Edge> predicate = parse_edge(finds[1], edge_cache);
